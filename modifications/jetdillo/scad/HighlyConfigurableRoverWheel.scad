module sawppy_hub() { 
    ...
    translate([-3,0,0]) scale([1.05,1.05,1.1]) 
    import("/home/armadilo/Documents/models/Sawppy_Rover/STL/Wheel_Hub_blank.stl",convexity=10);
    
}
  // rotate([0,90,0])  import("/home/armadilo/Documents/models/Sawppy_Rover/STL/Wheel.stl");
/*
...
wheelWidth     = 55;     // The width (or thickness) of the wheel at the rim.
tireCSDiameter = 6;     // Cross-sectional diameter (CS) -- How thick is the tire rubber?
tireID         =110;    // Internal diameter (ID) -- How wide is the inside opening? 
...
knobSize          = [6,6,3];   // The size of each knob [across wheel, along the perimeter, prodruding]
                                  //   or for v-grooves, [angle, depth, ignored]
radialTreadSets   = 15;           // How many sets of treads to render around the wheel (2 rows per set).
numberOfKnobs     = 6;            // The number of knobs to render per row. 
staggerOffset     = 4;            // A distance to offset the staggered rows. 
...
hubDiameter          = 24;    // The diameter of the hub portion of the wheel
hubHeight            = 12;    // The total height of the hub
hubZOffset           = 4;     // The Z position of the hub, negative numbers from the surface of the wheel 
...
shaftFlatDiameter    = 8;          // The diameter of the motor shaft at the flat, or shaftDiameter for no flat.
...
servoHoleDistance1   = 0;         // Distance across servo horn from hole to hole (0 to ignore)
servoHoleDistance2   = 0;         // Distance across servo horn from hole to hole, rotated 90 degrees (0 to ignore)
servoArmRotation     = 0;         // The total rotation of all servo holes
...
difference() { 
 wheel();
    translate([0,0,0]) rotate([0,0,90]) { for (i=[0:3]) {
                
               translate([sin(360*i/3)*11,cos(360*i/3)*11,6]) cylinder(r=5,h=90,$fn=60,center=true);
     
             }
         }
  }
   translate([0,0,9]) scale([1.0,1.0,1.0]) sawppy_hub();
}
//sawppy_hub();
