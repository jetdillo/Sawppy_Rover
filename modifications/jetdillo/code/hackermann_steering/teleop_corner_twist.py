...
class Corner:
   ...
   def run(self): 
      ...
      while (not rospy.is_shutdown()):
         ...
         rear_newpos=0 
         if (self.twist.angular.z !=0): 
            front_newpos = self.twist.angular.z * 1.5757
            rear_newpos = front_newpos *-1 
         else:
            front_newpos = 0
         
            front_left_sp.publish(front_newpos)
            front_right_sp.publish(front_newpos)
            rear_left_sp.publish(rear_newpos)
            rear_right_sp.publish(rear_newpos)
